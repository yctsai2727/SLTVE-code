...
%metric
method = @(u) @(d1,d2) DisplacedWeightedTotalVariation(d1,d2,u,dx,dy);
...
for i = 0:m_p+1
    for j = 0:n_p+1
        ...
        down=0;
        right=0;
        if j==0 && i>0 %first row, cache bottom value
        ...
        elseif j>0 && j<n_p+1 %non-zero-th or n_p+1-th row
            ...
            elseif i>1 %second+ col, compute LTVE of prev col
...
if id_x==3
    sol = interp2(x(id_x*m_p+1:m,id_y*n_p+1:(id_y+1)*n_p)',y(id_x*m_p+1:m,id_y*n_p+1:(id_y+1)*n_p)',sol(1:m_p-3,1:n_p)',x_ad(id_x*m_p_ad+1:min((id_x+1)*m_p_ad,m_ad),id_y*n_p_ad+1:min((id_y+1)*n_p_ad,n_ad))',y_ad(id_x*m_p_ad+1:min((id_x+1)*m_p_ad,m_ad),id_y*n_p_ad+1:min((id_y+1)*n_p_ad,n_ad))',"cubic")';
else
    sol = interp2(x(id_x*m_p+1:(id_x+1)*m_p,id_y*n_p+1:(id_y+1)*n_p)',y(id_x*m_p+1:(id_x+1)*m_p,id_y*n_p+1:(id_y+1)*n_p)',sol',x_ad(id_x*m_p_ad+1:(id_x+1)*m_p_ad,id_y*n_p_ad+1:(id_y+1)*n_p_ad)',y_ad(id_x*m_p_ad+1:(id_x+1)*m_p_ad,id_y*n_p_ad+1:(id_y+1)*n_p_ad)',"cubic")';
end
x=x_ad;
y=y_ad;
m=m_ad;
...
for i = 0:m_p+1
    for j = 0:n_p+1
        ...
        down=0;
        right=0;
        if j==0 && i>0 && Filter(i,j+1)==1  %zero-th row and point below is anchor, cache bottom value
            cache(n_p+1) = MatTrajMetric({traj(j+1,:,:,:),id_x*m_p+i-1,id_y*n_p+j},{traj(j+2,:,:,:),id_x*m_p+i-1,id_y*n_p+j+1},K,method(2),decoder);
        elseif j>0 && j<n_p+1 %non-zero-th or n_p+1-th row
            if i==1 && Filter(i,j)==1 %first col, lack of right info, only cache value
                cache(j) = MatTrajMetric({traj(j+1,:,:,:),id_x*m_p+i-1,id_y*n_p+j},{curr,id_x*m_p+i,id_y*n_p+j},K,method(1),decoder);
            elseif i>1 && Filter(i-1,j)==1
                right = MatTrajMetric({traj(j+1,:,:,:),id_x*m_p+i-1,id_y*n_p+j},{curr,id_x*m_p+i,id_y*n_p+j},K,method(1),decoder);
                down = MatTrajMetric({traj(j+1,:,:,:),id_x*m_p+i-1,id_y*n_p+j},{traj(j+2,:,:,:),id_x*m_p+i-1,id_y*n_p+j+1},K,method(2),decoder);
                sol(i-1,j) = max(max(cache(n_p+1),down),max(cache(j),right));
                cache(j) = right;
...
[str]=strcat("./ParallelSol/DoubleGyre_MatDisSqrtWeightTVnormParaCompu257x128adapD000001HighRank_",int2str(id),".mat");
